...
public:
    static void log(const std::string& message) {
        ...
        if (monitor_socket_fd != -1) {
            ...
            if (send(monitor_socket_fd, log_message.c_str(), log_message.length(), 0) < 0) {
                std::cerr << "Ошибка отправки лога в монитор: " << strerror(errno) << std::endl;
            }
        }
    }
};
...
int main(int argc, char *argv[]) {
    ...
    while (connected_clients < 3) {
        ...
        if (message == "client\n") {
            ...
            monitor_socket_fd = client_socket;
